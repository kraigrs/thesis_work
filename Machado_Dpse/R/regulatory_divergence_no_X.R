...
#data <- read.table("/Users/kraigrs/Wittkopp/Machado_Dpse/data/TL_Toro1_H6_M_carcass.genes_down_ASE.txt",header=TRUE,sep="\t");
#dir <- "/Users/kraigrs/Wittkopp/Machado_Dpse/plots";
#out <- "TL_Toro1_H6_M_carcass";
...
data_annotation <- merge(data_cutoff,annotation,by.x="gene",by.y="gene");
temp <- data_annotation[grep("^[^X]",data_annotation$chromosome,perl=TRUE),];
...
reg_div <- apply(as.matrix(data_results[,c(2,3,4,5,23,24,25)]),1,regulatory_divergence);

data_final <- cbind(data_results,percent_cis,reg_div);

write.table(data_final,file=paste(dir,"/",out,"_reg_div_no_X.txt",sep=""),quote=FALSE,sep="\t",row.names=FALSE);
...
pdf(file=paste(dir,"/",out,"_percent_cis_reg_div_no_X.pdf",sep=""));
...
pdf(file=paste(dir,"/",out,"_reg_div_no_X_pie.pdf",sep=""));
...
pdf(file=paste(dir,"/",out,"_reg_div_no_X.pdf",sep=""));
