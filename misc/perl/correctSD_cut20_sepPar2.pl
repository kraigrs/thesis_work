...
#
# E.g. perl correctSD_cut20_sepPar2.pl <species1> <species2> <species1file> <s1depth> <species2file> <s2depth> <out_dir>
#
...
sub main
{
  ...
  open(PAR,">$out_dir/$species1\_$species2.SD_cut20_sepPar2.$suffix") or die "Error writing to $out_dir/$species1\_$species2.SD_cut20_sepPar.$suffix\n";
  ...
  foreach $locus (keys %loci)   
  {
    if($par1{$locus} && $par2{$locus} && $par1{$locus}[0] + $par2{$locus}[1] >= 20)
    {
      printf PAR ("\n$locus\t%.0f\t%.0f\t%.0f",
                  ...
                  int($par1{$locus}[2]*$factorS1+$par2{$locus}[2]*$factorS2+0.5));
    }
  }
  close PAR;
}
