...
#
# E.g. perl correctMB_cut20_sepPar.pl <species1> <species2> <species1file> <species2file> <out_dir> <suffix>
#
...
sub main
{
  ...
  foreach $locus (keys %loci)   
  {
    ...
    {
      $parS1  += $par1{$locus}; $parS2  += $par2{$locus};
    }
  }
  ...
  open(PAR,">$out_dir/$species1\_$species2.MB_cut20_sepPar.$suffix") or die "Error writing to $out_dir/$species1\_$species2.MB_cut20_sepPar2.$suffix\n";
  ...
  foreach $locus (keys %loci)   
  {
    ...
    {
      printf PAR ("\n$locus\t%.1f\t%.1f",
                  floor($par1{$locus}*$factorParS1),
                  floor($par2{$locus}*$factorParS2));
    }   
  }
  close PAR;
}
