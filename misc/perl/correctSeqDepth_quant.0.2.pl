...
#
# E.g. perl processData.pl <species1> <species2> <species1file> <hyb1file> <hyb1depth> <hyb2file> <hyb2depth> <out_dir>
#
...
sub main
{
  ...
  my$factorS1; my$factorS2; my$factorHyb1; my$factorHyb2;
  ...
  foreach $locus (keys %loci)   
  {
    ...
    {
      push @parHyb1s1cts,$par{$locus}[0];
      push @parHyb1s2cts,$par{$locus}[1];
    }
    if($par{$locus} && $hyb2{$locus})
    {
      push @parHyb2s1cts,$par{$locus}[0];
      push @parHyb2s2cts,$par{$locus}[1];
    }
  }
  ...
  open(PARHYB1,">$out_dir/$species1\_$species2.parHyb1_seqDepth_quant.$suffix") or die "Error writing to $out_dir/$species1\_$species2.parHyb1.$suffix\n";
  open(PARHYB2,">$out_dir/$species1\_$species2.parHyb2_seqDepth_quant.$suffix") or die "Error writing to $out_dir/$species1\_$species2.parHyb2.$suffix\n";
  open(HYB1HYB2,">$out_dir/$species1\_$species2.hyb1hyb2_seqDepth_quant.$suffix") or die "Error writing to $out_dir/$species1\_$species2.hyb1hyb2.$suffix\n";
  ...
  {
    if($par{$locus} && $hyb1{$locus} && 
       $par{$locus}[0] > $LBparHyb1s1 && $par{$locus}[0] < $UBparHyb1s1 &&
       $par{$locus}[1] > $LBparHyb1s2 && $par{$locus}[1] < $UBparHyb1s2)
    {
      printf PARHYB1 ("\n$locus\t%.0f\t%.0f\t%.0f\t%.0f\t%.0f\t%.0f",
                      $par{$locus}[0],$par{$locus}[1],$par{$locus}[2],
                      $hyb1{$locus}[0],$hyb1{$locus}[1],$hyb1{$locus}[2]);
    }
    if($par{$locus} && $hyb2{$locus} && 
       $par{$locus}[0] > $LBparHyb2s1 && $par{$locus}[0] < $UBparHyb2s1 &&
       $par{$locus}[1] > $LBparHyb2s2 && $par{$locus}[1] < $UBparHyb2s2)
    {
      printf PARHYB2 ("\n$locus\t%.0f\t%.0f\t%.0f\t%.0f\t%.0f\t%.0f",                      
                      $par{$locus}[0],$par{$locus}[1],$par{$locus}[2],
