...
sub main
{
  my$species1 = $ARGV[0]; 
  my$species2 = $ARGV[1];
  my$name1 = $ARGV[2];  # e.g. zhr+z30
  my$par1file = $ARGV[3];
  my$s1depth = $ARGV[4];
  my$name2 =$ARGV[5] ;  # e.g. zhrXz30
  my$par2file = $ARGV[6];
  my$s2depth = $ARGV[7];
  my$output = $ARGV[8];
  ...
  # separate parents
  #print OUT "locus\t$species1\t$species2\tBoth";
  ...
  foreach $locus (keys %loci)   
  {
    ...
    # use this for comparing any other 2 files (separate parents are special case, see above)
    if($par1{$locus} && $par2{$locus} && $par1{$locus}[0] + $par1{$locus}[1] >= 20 && $par2{$locus}[0] + $par2{$locus}[1] >= 20)
    {
