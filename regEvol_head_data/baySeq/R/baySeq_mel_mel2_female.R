...
####################################################################################################
####################################################################################################
#data analyses for mel-mel parent vs hybrid 2 female
####################################################################################################
####################################################################################################
...
i=1
data1<-read.delim("zhr_z30_F_r1.genes.txt",header = T)
mapcounts[i,1]=sum(data1[,2])
i=i+1
mapcounts[i,1]=sum(data1[,3])
i=i+1
data1<-read.delim("zhr_z30_F_r2.genes.txt",header = T)
mapcounts[i,1]=sum(data1[,2])
i=i+1
mapcounts[i,1]=sum(data1[,3])
i=i+1
data1<-read.delim("zhr_z30_F_r3.genes.txt",header = T)
mapcounts[i,1]=sum(data1[,2])
i=i+1
mapcounts[i,1]=sum(data1[,3])
i=i+1

data1<-read.delim("z30Xzhr_F_r1.genes.txt",header = T)
mapcounts[i,1]=sum(data1[,2])
i=i+1
mapcounts[i,1]=sum(data1[,3])
i=i+1
data1<-read.delim("z30Xzhr_F_r2.genes.txt",header = T)
mapcounts[i,1]=sum(data1[,2])
i=i+1
mapcounts[i,1]=sum(data1[,3])
i=i+1
data1<-read.delim("z30Xzhr_F_r3.genes.txt",header = T)
mapcounts[i,1]=sum(data1[,2])
i=i+1
mapcounts[i,1]=sum(data1[,3])
i=i+1
...
data1<-read.delim("zhr_z30_F_r1.genes.txt",header = T)
data2<-read.delim("zhr_z30_F_r2.genes.txt",header = T)
data3<-read.delim("zhr_z30_F_r3.genes.txt",header = T)


data4<-read.delim("z30Xzhr_F_r1.genes.txt",header = T)
data5<-read.delim("z30Xzhr_F_r2.genes.txt",header = T)
data6<-read.delim("z30Xzhr_F_r3.genes.txt",header = T)
...
save(CD.post, file = "mel_mel_P_H2_F_posteriors.bayseq")
save(CD.prior, file = "mel_mel_P_H2_F_priors.bayseq")

write.table(cbind(CD.post@annotation,CD.post@posteriors), file = "mel_mel_P_H2_F_posteriors.txt",quote=FALSE,row.names=FALSE,col.names=FALSE,sep="\t")
