...
####################################################################################################
####################################################################################################
#data analyses for mel-mel parent vs hybrid 1 male
####################################################################################################
####################################################################################################
...
i=1
data1<-read.delim("zhr_z30_M_r1.genes.txt",header = T)
mapcounts[i,1]=sum(data1[,2])
i=i+1
mapcounts[i,1]=sum(data1[,3])
i=i+1
data1<-read.delim("zhr_z30_M_r2.genes.txt",header = T)
mapcounts[i,1]=sum(data1[,2])
i=i+1
mapcounts[i,1]=sum(data1[,3])
i=i+1
data1<-read.delim("zhr_z30_M_r3.genes.txt",header = T)
mapcounts[i,1]=sum(data1[,2])
i=i+1
mapcounts[i,1]=sum(data1[,3])
i=i+1
...
data1<-read.delim("zhr_z30_M_r1.genes.txt",header = T)
data2<-read.delim("zhr_z30_M_r2.genes.txt",header = T)
data3<-read.delim("zhr_z30_M_r3.genes.txt",header = T)


data4<-read.delim("zhrXz30_M_r1.genes.txt",header = T)
data5<-read.delim("zhrXz30_M_r2.genes.txt",header = T)
data6<-read.delim("zhrXz30_M_r3.genes.txt",header = T)
...
save(CD.post, file = "mel_mel_P_H1_M_posteriors.bayseq")
save(CD.prior, file = "mel_mel_P_H1_M_priors.bayseq")

write.table(cbind(CD.post@annotation,CD.post@posteriors), file = "mel_mel_P_H1_M_posteriors.txt",quote=FALSE,row.names=FALSE,col.names=FALSE,sep="\t")
