...
# 
# Input: reads, reference
#
# Output: alignments
#
# Syntax: perl mosaikPipe.0.2.pl <ref> <reads>
#
########################################################################
...
#exit;
system "MosaikAligner -in $matlab\.dat -out $matlab\.$refname -ia $label\.dat -om -hs 14 -mm 0 -m unique -j $label\_14 -p 2 -annse 2.1.26.se.100.005.ann -annpe 2.1.26.pe.100.0065.ann > $matlab\.$refname\.output";

#system "MosaikAligner -in $matlab\.dat -out $matlab\.$refname\.dat -ia $label\.dat -hs 14 -mm 0 -m unique -j $label\_14 -p 2 -rur $matlab\.$refname\.un -bw 17 -act 25 > $matlab\.$refname\.output";
