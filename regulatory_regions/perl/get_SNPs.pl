...
#
# Output: for each entry in BED file, the actual sequence from the reference (tweaked the output a little bit)
#
...
foreach $chr (keys %regs)
{
  foreach $locus (keys %{$regs{$chr}})
  {
    if($genome{$chr})
    {
      ...
      print OUT "$chr\t$regs{$chr}{$locus}[0]\t$regs{$chr}{$locus}[1]\t$locus\t$string\n";
    }
  }
}
close OUT;
