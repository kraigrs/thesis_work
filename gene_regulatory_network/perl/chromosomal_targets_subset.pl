...
print "target\tchromosome\tregulators\tchr$chromosome\_regs\n";
...
foreach $target (keys %network)
{
  ...
  #if($chroms{$target})
  {
    ...
    foreach $regulator ( @{ $network{$target} } )
    {
      ...
      #if($chroms{$regulator})
      {
        ...
        if($chroms{$regulator} eq $chromosome){$Xreg += 1;}
      }
    }

    print "$FBgn2CG{$target}\t$chroms{$target}\t$n\t$Xreg\n";
